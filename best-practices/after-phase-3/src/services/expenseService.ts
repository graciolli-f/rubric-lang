...
import { convertFileToBase64, ReceiptError } from './receiptService';
import { getUserPreferences, fetchExchangeRates, convertCurrency } from './currencyService';
 ...
 */
export async function formDataToCreateData(formData: ExpenseFormData): Promise<ExpenseCreateData> {
  ...
  if (formData.receipt) {
    try {
      receiptBase64 = await convertFileToBase64(formData.receipt);
    } catch (error) {
      if (error instanceof ReceiptError) {
        throw new ValidationError(`Receipt error: ${error.message}`);
      }
      throw new ValidationError('Failed to process receipt image');
    }
  }
...
export async function createExpense(data: ExpenseCreateData): Promise<Expense> {
  ...
  try {
    ...
    if (userPrefs.preferredCurrency !== data.currency) {
      ...
      displayCurrency = userPrefs.preferredCurrency;
    }
  } catch (error) {
    // If currency conversion fails, use original values
...
export function expenseToFormData(expense: Expense): ExpenseFormData {
  return {
    ...
    receipt: null, // Can't convert base64 back to File
